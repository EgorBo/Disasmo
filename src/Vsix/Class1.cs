...
namespace Disasmo
{
    ...
    [Name("Git commit details UI factory 2")]
    [TypeConversion(from: typeof(string), to: typeof(FrameworkElement))]
    [Order]
    internal class ViewElementFactory : IViewElementFactory
    {
        public TView CreateViewElement<TView>(ITextView textView, object model) where TView : class
        {
            ...
            if (model is string)
            {
                var detailsUI = new DisasmLensUI();
                detailsUI.DataContext = model;
